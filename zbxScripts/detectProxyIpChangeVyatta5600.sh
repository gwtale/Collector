...
if [ "$OLDIP" != "$NEWIP" ]; then
    cp /var/Collector/zbxScripts/updateProxyIPVyatta5600.exp /tmp/updateProxyIPVyatta5600_$IP.exp
    /tmp/updateProxyIPVyatta5600_$IP.exp $IP $ID $PW
    sleep 5
    ifconfig tun0 | grep "inet addr:" | cut -d: -f2 | awk '{print $1}' > /tmp/zbxProxyIp_$IP
    rm /tmp/updateProxyIPVyatta5600_$IP.exp
