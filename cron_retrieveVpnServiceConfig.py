...
with conn.cursor() as cursor:
    sql = "SELECT `account_userID`, `account_passwd`, `account_vpnID`, `account_vpnPassword`  FROM `customer_accounts` WHERE `account_id`=%s"
    cursor.execute(sql, (base["account_id"],))
    result = cursor.fetchone()
    if result == None:
	logger.error('Empty dataset when querying for VPN and API credentials on table customer_accounts for account_id='+str(base["account_id"]))
    else:
